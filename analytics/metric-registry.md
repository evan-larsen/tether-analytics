...
| Habit-forming users | Highest qualifying reader-habit tier: at least 3 distinct active days in the preceding 7 complete days | Pull from virtual `cohort_habit_forming_users`; group by `snapshot_day` | `user_analytics_id` | Exclusive rolling-state tier: excludes Active, Regular, and Near-daily users; backed by the hourly 90-day materialized source. |
| Active readers | Highest qualifying reader-habit tier: at least 7 distinct active days in the preceding 14 complete days | Pull from virtual `cohort_habit_active_readers`; group by `snapshot_day` | `user_analytics_id` | Exclusive rolling-state tier: excludes Regular and Near-daily users; backed by the hourly 90-day materialized source. |
| Regular readers | Highest qualifying reader-habit tier: 18–24 distinct active days in the preceding 30 complete days | Pull from virtual `cohort_habit_regular_readers`; group by `snapshot_day` | `user_analytics_id` | Exclusive rolling-state tier backed by the hourly 90-day materialized source, which scans 120 days of source events. |
