...
| Habit-forming users | Users who started a reading session on at least 3 distinct days in the preceding 7 complete days | Pull from `cohort_habit_forming_users`; group by `snapshot_day` | `user_analytics_id` | Historical rolling-state cohort with 90 snapshot days; excludes the active Utah-time day, so membership is not permanent and must be joined at the intended snapshot date. |
| Active readers | Users who started a reading session on at least 7 distinct days in the preceding 14 complete days | Pull from `cohort_habit_active_readers`; group by `snapshot_day` | `user_analytics_id` | Historical rolling-state cohort with 90 snapshot days; excludes the active Utah-time day and has a 104-day source-event scan. |
| Regular readers | Users who started a reading session on at least 15 distinct days in the preceding 30 complete days | Pull from `cohort_habit_regular_readers`; group by `snapshot_day` | `user_analytics_id` | Historical rolling-state cohort with 90 snapshot days; excludes the active Utah-time day and conservatively scans 120 days of source events. |
| Near-daily readers | Users who started a reading session on at least 25 distinct days in the preceding 30 complete days | Pull from `cohort_habit_near_daily_readers`; group by `snapshot_day` | `user_analytics_id` | Historical rolling-state cohort with 90 snapshot days; excludes the active Utah-time day and conservatively scans 120 days of source events. |
