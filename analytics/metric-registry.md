...
| --- | --- | --- | --- | --- |
| New users | First measurable onboarding entrants | First `app_opened` where `is_in_onboarding = true` | `distinct_id` | Anonymous top-of-funnel metric, not canonical-user creation |
...
| Executive D1 retention | Onboarding entrants who return on exact day 1 | First onboarding `app_opened`; any later event on day 1 | `distinct_id` | Preserve current Executive semantics exactly |
