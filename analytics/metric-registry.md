...
| Pre-habit daily active readers | Canonical DAU not assigned to a reader-habit tier | Distinct `user_analytics_id` on `reading_session_completed`, minus the four exclusive tiers on the same `snapshot_day` | `user_analytics_id` | Dashboard-only DAU segment; may include brand-new or returning low-frequency readers and is not the acquisition `New users` cohort. |
